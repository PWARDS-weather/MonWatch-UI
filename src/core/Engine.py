...
from .products import RGB_PRODUCTS
...
class EmbeddedRGBEngine:
    ...
    def _read_band_array_locked(file_to_open, band, max_px=0):
        ...
        if isinstance(file_to_open, Path) and file_to_open.suffix.lower() == ".nc":
            from src.parsers.mtsat_parser import is_mtsat_file, read_mtsat_band_data
            if is_mtsat_file(file_to_open):
                arr, _, _ = read_mtsat_band_data(file_to_open, band, max_px=max_px)
                if arr is not None:
                    return arr
                return None

        if isinstance(file_to_open, Path):
            from src.parsers.hrit_parser import is_hrit_file, read_hrit_band_data
            if is_hrit_file(file_to_open):
                arr, _, _ = read_hrit_band_data(file_to_open, band, max_px=max_px)
                return arr
    ...
    def band_as_image(cls, nc_path: Path, band: str, max_px: int = 2200,
        ...
        try:
            ...
            arr = cls._read_band_array(file_to_open, band, max_px=max_px)
            if arr is None:
                return None
    ...
    def _sandwich_sataid_ir_lookup(bt_kelvin: np.ndarray) -> np.ndarray:
        ...
        # Invert the cold/warm color order: -73.15 C (coldest tops) now maps to
        # dark red and -33.15 C to dark blue. The grey-to-black ramp warmer than
        # -33.15 C is applied separately below and is intentionally NOT reversed.
        lut = lut[::-1].copy()
        lut /= 255.0
    ...
    def _true_color_daynight(cls, cache: dict, nc_path: Path = None, max_px: int = 0) -> np.ndarray:
        ...
        if _needs_div:
            b03 = use_xp.clip(b03_raw / 100.0, 0.0, 3.0)
            b02 = use_xp.clip(b02_raw / 100.0, 0.0, 3.0)
            b01 = use_xp.clip(b01_raw / 100.0, 0.0, 3.0)
        else:
    ...
    def _compute_solar_zenith_angle(nc_path, h, w):
        ...
        try:
            ...
            from src.parsers.hrit_parser import is_hrit_file, extract_hrit_crs, parse_hrit_metadata
            if is_hrit_file(_orig_path):
                crs, gt = extract_hrit_crs(_orig_path)
                if crs is None or gt is None:
                    return np.zeros((h, w), dtype=np.float32)
                obs_dt = None
                meta = parse_hrit_metadata(_orig_path)
                if meta.get("timestamp"):
                    m = re.search(r'(\d{4})_(\d{2})_(\d{2})_(\d{2})(\d{2})', meta["timestamp"])
                    if m:
                        from datetime import datetime, timezone
                ...
                ncols = int(round(abs(gt[2]) / gt[0])) * 2
                nrows = ncols
                x = gt[2] + np.arange(ncols) * gt[0]
                y = gt[5] + np.arange(nrows) * gt[4]
                step_y = max(1, nrows // h)
                step_x = max(1, ncols // w)
                y_sub = y[::step_y][:h]
                x_sub = x[::step_x][:w]
                    ...
                    for attr in ('longitude_of_projection_origin', 'satellite_longitude',
                        ...
                        if not sidecar.exists():
                            ...
                            for _f in nc_path.parent.glob("AHI_*.ads.json"):
                                if _any_sc is None:
                                    _any_sc = _f
                                if _token is not None and _token in _f.name:
                                    sidecar = _f
                                    break
                            if not sidecar.exists() and _any_sc is not None:
                                sidecar = _any_sc
                        if sidecar.exists():
                            try:
                                import json
