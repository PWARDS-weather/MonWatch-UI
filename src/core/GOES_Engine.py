...
from .GOES_Products import RGB_PRODUCTS
...
class EmbeddedRGBEngine:
    ...
    def composite_realtime(cls, product_key: str, nc_path: Path = None,
        ...
        if not info.get("single_band") and info.get("special") not in ("sandwich", "geocolor", "true_daynight", "geonat_daynight", "false_color", "false_color_adv", "true_color_unidata"):
            if max_px == 0:
        ...
        if info.get("special") == "geonat_daynight":
            return cls._geonat_color_daynight(cache, nc_path, max_px=max_px)
    ...
    def _true_color_daynight(cls, cache: dict, nc_path: Path = None, max_px: int = 0) -> np.ndarray:
        ...
        _nan_alpha = (use_xp.isnan(b03_raw) | use_xp.isnan(b02_raw) | 
              use_xp.isnan(b01_raw) | use_xp.isnan(ir_raw))
        ...
        _max_val = float(use_xp.nanmax(use_xp.array([b03_raw, b02_raw, b01_raw])))
        _needs_div = _max_val > 1.0
        if _needs_div:
        ...
        b03_day = use_xp.clip(b03 * path_sun_a, 0.0, 1.0)
        b02_day = use_xp.clip(b02 * path_sun_a, 0.0, 1.0)
        b01_day = use_xp.clip(b01 * path_sun_a, 0.0, 1.0)
        ...
        g_day = 0.45 * b03_corr + 0.1 * b02_corr + 0.45 * b01_corr
        day_rgb = use_xp.stack([b03_corr, g_day, b01_corr], axis=-1)
        day_rgb = use_xp.power(day_rgb, 1.0 / 2.2)
        day_rgb = day_rgb * day_weight[:, :, None]
    ...
    @classmethod
    def _geonat_color_daynight(cls, cache: dict, nc_path: Path = None, max_px: int = 0) -> np.ndarray:
        import time as _time
        _t0 = _time.time()
        use_xp = cls._active_xp()
        _xp_name = "cupy" if hasattr(use_xp, "cuda") else "numpy"
        print(f"[GN_DN] start | xp={_xp_name} | bands in cache: {list(cache.keys())}")

        print(f"[GN_DN] shapes: B13={cache['B13'].shape}, B03={cache['B03'].shape}, B02={cache['B02'].shape}, B01={cache['B01'].shape}")
        if max_px == 0:
            _target_band = max(["B13", "B03", "B02", "B01"], key=lambda b: cache[b].shape[0] * cache[b].shape[1])
        else:
            _target_band = min(["B13", "B03", "B02", "B01"], key=lambda b: cache[b].shape[0] * cache[b].shape[1])
        target_h, target_w = cache[_target_band].shape
        print(f"[GN_DN] target grid: {target_h}x{target_w}")
        for b in ("B13", "B03", "B02", "B01"):
            arr = cache[b]
            if arr.shape != (target_h, target_w):
                zh = target_h / arr.shape[0]
                zw = target_w / arr.shape[1]
                print(f"[GN_DN] zooming {b}: {arr.shape} -> {target_h}x{target_w} (zh={zh:.4f}, zw={zw:.4f})")
                arr = zoom(arr, (zh, zw), order=1)
                cache[b] = arr
        ...
        b02_raw = use_xp.where(use_xp.isnan(b02_raw), use_xp.float32(0), b02_raw)
        b01_raw = use_xp.where(use_xp.isnan(b01_raw), use_xp.float32(0), b01_raw)
        print(f"[GN_DN] NaN fill done | alpha_both={int(_nan_alpha.sum())}/{h*w}")

        scale13, off13 = cls._get_band_scale_offset(nc_path, "B13") if nc_path else (1.0, 0.0)
        ir = ir_raw * scale13 + off13
        ir = use_xp.where(use_xp.isnan(ir), use_xp.float32(300.0), ir)
        print(f"[GN_DN] B13 scale={scale13:.6f} off={off13:.6f}")
        ...
        if nc_path is not None:
            sza = cls._compute_solar_zenith_angle(nc_path, h, w)
        if sza is None:
            print(f"[GN_DN] SZA unavailable, falling back to zeros (all day)")
            sza = np.zeros((h, w), dtype=np.float32)
        sza = cls._to_xp(sza)
        sza = use_xp.nan_to_num(sza, nan=90.0)
        print(f"[GN_DN] SZA shape={sza.shape}")
        ...
        _day_px = int((day_weight > 0.5).sum())
        _night_px = int((day_weight < 0.5).sum())
        print(f"[GN_DN] day_px={_day_px} night_px={_night_px}")
        ...
        b02_corr = use_xp.clip(b02_day - rayleigh_g, 0.0, 1.0)
        b01_corr = use_xp.clip(b01_day - rayleigh_b, 0.0, 1.0)
        day_rgb = use_xp.stack([b03_corr, b02_corr, b01_corr], axis=-1)
        day_rgb = use_xp.power(day_rgb, 1.0 / 2.2)
        day_rgb = day_rgb * day_weight[:, :, None]
        ...
        rgb = day_rgb + night_rgb
        print(f"[GN_DN] blended rgb range=[{float(use_xp.nanmin(rgb)):.4f},{float(use_xp.nanmax(rgb)):.4f}]")
        ...
        b_final = use_xp.clip(luminance + saturation_factor * (rgb[:,:,2] - luminance), 0.0, 1.0)
        rgb = use_xp.stack([r_final, g_final, b_final], axis=-1)
        print(f"[GN_DN] sat_boost applied factor={saturation_factor}")

        rgb_u8 = (use_xp.clip(rgb, 0.0, 1.0) * 255).astype(use_xp.uint8)
        alpha_ch = use_xp.where(_nan_alpha, use_xp.uint8(0), use_xp.uint8(255))
        rgba_u8 = use_xp.concatenate([rgb_u8, alpha_ch[:, :, None]], axis=-1)
        _t1 = _time.time()
        print(f"[GN_DN] done in {_t1-_t0:.3f}s | rgba_u8 shape={rgba_u8.shape}")
        return cls._to_np(rgba_u8)
    ...
    def _compute_solar_zenith_angle(nc_path, h, w):
        ...
        try:
            ...
            # If nc_path is a synthetic per-band file, try to find original GOES NC file
            _orig_path = nc_path
            try:
                _p = Path(nc_path)
